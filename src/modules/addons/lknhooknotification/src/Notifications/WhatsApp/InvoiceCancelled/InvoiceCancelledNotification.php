...
/**
 * Code: InvoiceCancelled
 */

namespace Lkn\HookNotification\Notifications\WhatsApp\InvoiceCancelled;
 ...
 * @since 3.3.0
 */
final class InvoiceCancelledNotification extends AbstractWhatsAppNotifcation
{
    public string $notificationCode = 'InvoiceCancelled';

    /**
     * @var \Lkn\HookNotification\Config\Hooks|array|null
     * @link https://developers.whmcs.com/hooks-reference/cron/#dailycronjob
     */
    public Hooks|array|null $hook = Hooks::INVOICE_CANCELLED;
    ...
    public function settings(array $newSettings = []): array
    {
        if (empty($newSettings)) {
            $savedSettings = $this->getSettings();

            $settings = [
                'send_when_order_cancelled' => $savedSettings['send_when_order_cancelled'] ?? false
            ];
        } else {
            $settings = [
        ...
        return [
            [
                'id' => 'send_when_order_cancelled',
                'label' => $this->lang['settings']['send_when_order_cancelled']['label'],
                'descrip' => $this->lang['settings']['send_when_order_cancelled']['descrip'],
                'type' => 'checkbox',
                'value' => $settings['send_when_order_cancelled']
            ]
        ];
    }
}
