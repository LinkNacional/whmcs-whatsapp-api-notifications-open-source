...
final class QuoteChangedRegistered extends WhatsappHookFile
{
    /**
     * @since 2.0.0
     *
    ...
    public function run($hookData): bool
    {
        ...
        $templateData = $this->getTemplateForHook('QuoteChangedRegistered');
        ...
        if ($response['success']) {
            (new ChatwootSendMessageAsPrivate())->run(
                $hookData->clientId,
                'Mensagem sobre orçamento #' . $hookData->quoteId . ' alterado foi enviada para este cliente.'
            );
        }
